...
* ***Models***
    ...
    * The reason to increase $\beta_t$ for smaller $T$ is to make $q(x_T | x_0)$ close to platent. 
...
* ***Interpolation in latent space***
    ...
    * We first sample $x^a_t ∼ q(x_t|x^a_0)$ and $x^b_t ∼ q(x_t|x^b_0)$ for the two speakers. 
    * We then do linear interpolation between x^a_t and x^b_t
    $$x^\lambda_t = (1-\lambda)x^a_t + \lambda x^b_t,     \text{for}  0<\lambda<1$$
    * Finally, we sample $x^\lambda_0 ∼ p_\theta(x^\lambda_0 |x^\lambda_t)$. The audio samples are in Section VI on the demo website.
